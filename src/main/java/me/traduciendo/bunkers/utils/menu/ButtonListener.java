package me.traduciendo.bunkers.utils.menu;

import me.traduciendo.bunkers.Bunkers;
import me.traduciendo.bunkers.utils.menu.pagination.PaginatedMenu;
...
public class ButtonListener implements Listener {
    ...
    if (openMenu != null) {
      ...
      if (openMenu.getButtons().containsKey(event.getSlot())) {
        ...
        if (event.isCancelled()) {
          Bukkit.getScheduler().runTaskLater(Bunkers.getInstance(), player::updateInventory, 1L);
        }
      } else {
  ...
  public void onInventoryClose(InventoryCloseEvent event) {
    ...
    player.setMetadata("scanglitch", new FixedMetadataValue(Bunkers.getInstance(), true));
  }
  ...
  public void onPlayerMove(PlayerMoveEvent event) {
    ...
    if (player.hasMetadata("scanglitch")) {
      player.removeMetadata("scanglitch", Bunkers.getInstance());
